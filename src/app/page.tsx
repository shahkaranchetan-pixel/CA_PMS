...
function StatsSkeleton() {
    return (
        <div className="stats-dashboard">
            {[1, 2, 3, 4].map(i => <div key={i} className="scard" style={{ height: '110px', background: 'var(--surface2)', animation: 'pulse 1.5s infinite ease-in-out' }} />)}
        </div>
    );
}
function ComplianceSkeleton() {
    return <div className="card" style={{ height: '160px', marginBottom: '24px', background: 'var(--surface2)', animation: 'pulse 1.5s infinite ease-in-out' }} />;
}
function MyTasksSkeleton() {
    return <div style={{ height: '300px', marginBottom: '24px', background: 'var(--surface)', borderRadius: '14px', animation: 'pulse 1.5s infinite ease-in-out' }} />;
}
function CalendarSkeleton() {
    return <div className="card" style={{ height: '200px', marginBottom: '16px', background: 'var(--surface2)', animation: 'pulse 1.5s infinite ease-in-out' }} />;
}
function BottomWidgetsSkeleton() {
    return (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '16px' }}>
            {[1, 2, 3].map(i => <div key={i} className="card" style={{ height: '350px', background: 'var(--surface2)', animation: 'pulse 1.5s infinite ease-in-out' }} />)}
        </div>
    );
}
...
async function StatsRow({ baseTaskWhere, displayMonth, displayYear }: DashboardProps) {
    const now = new Date();
    const [totalClients, overdueTasksCount, completedThisMonth, blockedTasksCount, underReviewTasksCount] = await Promise.all([
...
async function ComplianceHealth({ baseTaskWhere, displayMonth, displayYear, userRole }: DashboardProps) {
    const currentPeriod = `${monthNames[displayMonth].substring(0, 3)}-${displayYear}`;
    const now = new Date();
    
    const [statutoryTasksCount, completedThisMonth, overdueTasksCount] = await Promise.all([
        prisma.task.count({
            where: { ...baseTaskWhere, period: currentPeriod, frequency: 'MONTHLY', deletedAt: null }
        }),
        prisma.task.count({ where: { ...baseTaskWhere, deletedAt: null, status: 'COMPLETED', updatedAt: { gte: new Date(displayYear, displayMonth, 1) } } }),
        prisma.task.count({ where: { ...baseTaskWhere, deletedAt: null, status: { not: 'COMPLETED' }, dueDate: { lt: now } } })
    ]);
...
async function NewlyOnboarded({ userRole }: { userRole: string }) {
...
async function MyTasksList({ baseTaskWhere }: { baseTaskWhere: any }) {
    const myTasks = await prisma.task.findMany({
        where: { ...baseTaskWhere, deletedAt: null },
        select: {
    ...
    return (
        <div style={{ marginBottom: '24px' }}>
            <MyTasksDashboard tasks={myTasks as any} />
        </div>
    );
}

async function MiniCalendar({ baseTaskWhere, displayMonth, displayYear }: DashboardProps) {
...
async function BottomWidgets({ baseTaskWhere, userRole }: DashboardProps) {
    const now = new Date();
    
    const [upcomingDeadlines, recentLogs, team, recentEscalations] = await Promise.all([
        prisma.task.findMany({
            where: { ...baseTaskWhere, deletedAt: null, status: { not: 'COMPLETED' }, dueDate: { gte: now } },
    ...
    return (
        <div>
            ...
            <Suspense fallback={<StatsSkeleton />}>
                <StatsRow {...dashboardProps} />
            </Suspense>

            <Suspense fallback={<ComplianceSkeleton />}>
                <ComplianceHealth {...dashboardProps} />
            </Suspense>

            <Suspense fallback={null}>
                <NewlyOnboarded userRole={userRole} />
            </Suspense>

            <Suspense fallback={<MyTasksSkeleton />}>
                <MyTasksList baseTaskWhere={baseTaskWhere} />
            </Suspense>

            <Suspense fallback={<CalendarSkeleton />}>
                <MiniCalendar {...dashboardProps} />
            </Suspense>

            <Suspense fallback={<BottomWidgetsSkeleton />}>
                <BottomWidgets {...dashboardProps} />
            </Suspense>
            
            {/* Inject minimal CSS for skeleton pulse animations */}
            <style>{`
                @keyframes pulse {
                    0% { opacity: 0.4; }
                    50% { opacity: 0.8; }
